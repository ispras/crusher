...
- окружение и фаззинг-цель (Keycloak);
- сценарий работы Keycloak;
- начальный корпус вх. данных.

## 2.1. Окружение и фаззинг-цель
...
2. Создание докер-образа с фаззинг-целью.
...
3. Выгрузка `Keycloak` из докера на хост.
```shell
$ docker run --rm -v $PWD:$PWD -w $PWD -ti keycloak-fuzz /bin/bash
$ cp -r /root/fuzz/keycloak-26.0.7 .
$ exit
```
